...
wcat=pd.read_csv(r"C:\Users\smvs4\Desktop\Srinivas\Assignments\Simple linear regression\delivery_time.csv")
wcat.columns
wcat

plt.hist(wcat['Sorting Time'])
plt.boxplot(wcat['Sorting Time'],0,"rs",0)


plt.hist(wcat['Delivery Time'])
plt.boxplot(wcat['Delivery Time'])

plt.plot(wcat['Sorting Time'],wcat['Delivery Time'],"bo");plt.xlabel("Calories Consumed");plt.ylabel("Weight gained (grams)")

wcat['Sorting Time'].corr(wcat['Delivery Time'])

np.corrcoef(wcat['Sorting Time'],wcat['Delivery Time'])

# For preparing linear regression model we need to import the statsmodels.formula.api
import statsmodels.formula.api as smf
model=smf.ols("wcat['Sorting Time']~wcat['Delivery Time']",data=wcat).fit()
...
model2 = smf.ols("wcat['Sorting Time']~np.log(wcat['Delivery Time'])",data=wcat).fit()
model2.params
model2.summary()
...
model3 = smf.ols("np.log(wcat['Sorting Time'])~wcat['Delivery Time']",data=wcat).fit()
model3.params
model3.summary()
...
model_quad = smf.ols("wcat['Sorting Time']~wcat['Delivery Time']*wcat['Delivery Time']",data=wcat).fit()
model_quad.params
model_quad.summary()
