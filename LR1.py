...
wcat=pd.read_csv(r"C:\Users\smvs4\Desktop\Srinivas\Assignments\Simple linear regression\calories_consumed.csv")
wcat.columns
wcat
...
model=smf.ols("wcat['Weight gained (grams)']~wcat['Calories Consumed']",data=wcat).fit()
...
model2 = smf.ols("wcat['Weight gained (grams)']~np.log(wcat['Calories Consumed'])",data=wcat).fit()
model2.params
model2.summary()
...
model3 = smf.ols("np.log(wcat['Weight gained (grams)'])~(wcat['Calories Consumed'])",data=wcat).fit()
model3.params
model3.summary()
