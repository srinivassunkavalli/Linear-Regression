...
wcat=pd.read_csv(r"C:\Users\smvs4\Desktop\Srinivas\Assignments\Simple linear regression\Salary_Data.csv")
wcat.columns
wcat
wcat=pd.read_csv(r"C:\Users\smvs4\Desktop\Srinivas\Assignments\Simple linear regression\Salary_Data.csv")

plt.hist(wcat['Calories Consumed'])
plt.boxplot(wcat['Calories Consumed'],0,"rs",0)


plt.hist(wcat['Weight gained (grams)'])
plt.boxplot(wcat['Weight gained (grams)'])

plt.plot(wcat['Calories Consumed'],wcat['Weight gained (grams)'],"bo");plt.xlabel("Calories Consumed");plt.ylabel("Weight gained (grams)")

wcat['Calories Consumed'].corr(wcat['Weight gained (grams)'])

np.corrcoef(wcat['Calories Consumed'],wcat['Weight gained (grams)'])

# For preparing linear regression model we need to import the statsmodels.formula.api
import statsmodels.formula.api as smf
model=smf.ols("wcat['Salary']~wcat['YearsExperience']",data=wcat).fit()
...
pred = model.predict(wcat.iloc[:,-1]) # Predicted values of AT using the model

# Visualization of regresion line over the scatter plot of Waist and AT
# For visualization we need to import matplotlib.pyplot
import matplotlib.pylab as plt
plt.scatter(x=wcat['Calories Consumed'],y=wcat['Weight gained (grams)'],color='red');plt.plot(wcat['Calories Consumed'],pred,color='black');plt.xlabel('Calories Consumed');plt.ylabel('TISSUE')

pred.corr(wcat.AT) # 0.81

# Transforming variables for accuracy
model2 = smf.ols("wcat['Salary']~np.log(wcat['YearsExperience'])",data=wcat).fit()
model2.params
model2.summary()
...
model3 = smf.ols("np.log(wcat['Salary'])~wcat['YearsExperience']",data=wcat).fit()
model3.params
model3.summary()
...
student_resid
plt.plot(model3.resid_pearson,'o');plt.ax hline(y=0,color='green');plt.xlabel("Observation Number");plt.ylabel("Standardized Residual")
...
model_quad = smf.ols("wcat['Salary']~wcat['YearsExperience']+wcat['YearsExperience']+wcat['YearsExperience']*wcat['YearsExperience']",data=wcat).fit()
model_quad.params
model_quad.summary()
